...
func (cc *CryptoConfig) initFile(p ConfigParams) {
	resolveFile := func(path, src, name string) {
		...
		if err != nil {
			p.ErrStream <- fmt.Errorf(
				"failed to open key path '%s', source '%s' name '%s': %w",
				path, src, name, err,
			)
		}
		cc.f = f
	...
	default:
		p.ErrStream <- errors.New("failed to open key file, flag is required")
	}
}
...
func (cc *CryptoConfig) initData(errStream chan<- error) {
	...
	if err != nil {
		errStream <- fmt.Errorf("%s: %w", "failed to read key file", err)
		return
	}
	cc.Data = pemData
}
